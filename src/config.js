...
//   Flight numbers (see aircraft.js for how they are used):
//     speed   level top speed at sea level, full throttle (m/s)       ┐ the two calibration points: aircraft.js
//     mach    level top speed at 11 km, full throttle (Mach)          ┘ fits wave drag + ram thrust to hit both
//     cd0     subsonic zero-lift drag (game units: drag = 0.0016·lift·V²·ρ·cd0, m/s²). Supersonic types only;
//             the rest derive it from `speed`. Sets how fast the jet bleeds speed in turns / with the throttle back.
//     accel   full-afterburner (or full-power) thrust at sea level, standing still (m/s², i.e. T/W·g)
//     afterburner: false  jets without reheat (full power at 90% throttle, no flame)
//     lift    wing-loading multiplier · alpha = max AoA (deg) · gLimit · roll = max roll rate (rad/s)
//     aoaLimiter: true   fly-by-wire AoA limit: lift saturates at max AoA. Without it the wing stalls past
//             max AoA (lift loss + wing drop, recovered by easing the stick)
//
//   Top-speed targets (approximate published figures, clean; 1 Mach = 340 m/s at sea level):
//     type     SL Mach  11 km Mach    type     SL Mach  11 km Mach    type     SL Mach  11 km Mach
//     F-22      1.21     2.25         Su-35     1.14     2.25         F-5E      1.0      1.6
//     F-35A     1.1      1.6          MiG-29    1.23     2.25         Mirage    1.14     2.2
//     F-16C     1.2      2.05         MiG-31    1.23     2.83         Jaguar    1.1      1.6
//     F-15C     1.2      2.5          Typhoon   1.2      2.0          Su-47     1.13     1.65
//     F/A-18E   1.0      1.8          Rafale    1.13     1.8          B-2       0.85     0.95
//     F-14      1.2      2.34         J-20      1.15     2.0          Racer     0.68     0.72  (450 kt on the deck)
//     A-10C     0.62     0.7          Gripen    1.15     2.0          Pitts     0.28     -     (~185 kt)
//     F-4E      1.15     2.23         MiG-21    1.06     2.05         Cessna    0.19     -     (~126 kt)
//     F-2A      1.1      2.0          MiG-25    1.0      2.83 (1)     737/747   0.72/0.76 0.82/0.92 (MMO)
//     Su-57     1.15     2.0          J-10A     1.2      2.2          C-130J    0.5      0.6
//                                      J-8II     1.06     2.2
//   (1) MiG-25: ~M0.98 at low level (structural/thermal limit, modelled as a steep transonic drag wall),
//       so it only breaks through Mach 1 up high. Types that can't reach 11 km use the Mach there as a
...
export const AIRCRAFT = {
    f22: jet({
        ...
        flight: { speed: 411, mach: 2.25, cd0: 0.0478, aoaLimiter: true, accel: 16, gLimit: 9.5, roll: 4.4, alpha: 30, lift: 1.08 },
    ...
    f16: jet({
        ...
        flight: { speed: 408, mach: 2.05, cd0: 0.0521, aoaLimiter: true, accel: 15, gLimit: 9, roll: 5.0, alpha: 25, lift: 1.02 },
    ...
    f15: jet({
        ...
        flight: { speed: 408, mach: 2.5, cd0: 0.0422, aoaLimiter: true, accel: 15, gLimit: 9, roll: 3.8, alpha: 24, lift: 1.05 },
    ...
    fa18: jet({
        ...
        flight: { speed: 340, mach: 1.8, cd0: 0.0512, aoaLimiter: true, accel: 13, gLimit: 7.5, roll: 3.9, alpha: 40, lift: 1.1 },
    ...
    f14: jet({
        ...
        flight: { speed: 408, mach: 2.34, cd0: 0.0401, accel: 13, gLimit: 7.5, roll: 3.2, alpha: 26, lift: 1.0 },
    ...
    a10: jet({
        ...
        flight: { speed: 211, mach: 0.7, afterburner: false, accel: 8, gLimit: 7, roll: 2.6, alpha: 20, lift: 1.35 },
    ...
    f4: jet({
        ...
        flight: { speed: 391, mach: 2.23, cd0: 0.0488, accel: 13, gLimit: 7, roll: 3.0, alpha: 22, lift: 0.9 },
    ...
    f2: jet({
        ...
        flight: { speed: 374, mach: 2, cd0: 0.0493, aoaLimiter: true, accel: 14.5, gLimit: 9, roll: 4.6, alpha: 26, lift: 1.12 },
    ...
    su57: jet({
        ...
        flight: { speed: 391, mach: 2, cd0: 0.0474, aoaLimiter: true, accel: 15.5, gLimit: 9, roll: 4.3, alpha: 32, lift: 1.08 },
    ...
    su35: jet({
        ...
        flight: { speed: 388, mach: 2.25, cd0: 0.0446, aoaLimiter: true, accel: 14.5, gLimit: 9, roll: 3.9, alpha: 30, lift: 1.05 },
    ...
    mig29: jet({
        ...
        flight: { speed: 418, mach: 2.25, cd0: 0.0509, accel: 15, gLimit: 9, roll: 4.1, alpha: 28, lift: 1.02 },
    ...
    mig31: jet({
        ...
        flight: { speed: 418, mach: 2.83, cd0: 0.0435, accel: 16, gLimit: 5, roll: 2.6, alpha: 18, lift: 0.85 },
    ...
    typhoon: jet({
        ...
        flight: { speed: 408, mach: 2, cd0: 0.052, aoaLimiter: true, accel: 16, gLimit: 9, roll: 4.4, alpha: 28, lift: 1.04 },
    ...
    rafale: jet({
        ...
        flight: { speed: 384, mach: 1.8, cd0: 0.0531, aoaLimiter: true, accel: 15, gLimit: 9, roll: 4.3, alpha: 30, lift: 1.05 },
    ...
    j20: jet({
        ...
        flight: { speed: 391, mach: 2, cd0: 0.0478, aoaLimiter: true, accel: 14.5, gLimit: 8.5, roll: 3.8, alpha: 26, lift: 0.98 },
        health: 115, missiles: 8,
    ...
    gripen: jet({
        ...
        flight: { speed: 391, mach: 2, cd0: 0.0526, aoaLimiter: true, accel: 14.5, gLimit: 9, roll: 4.7, alpha: 28, lift: 1.05 },
    ...
    mig21: jet({
        ...
        flight: { speed: 360, mach: 2.05, cd0: 0.0597, accel: 13, gLimit: 7, roll: 3.8, alpha: 22, lift: 0.85 },
    ...
    mig25: jet({
        ...
        flight: { speed: 340, mach: 2.83, cd0: 0.0429, accel: 16, gLimit: 5, roll: 2.5, alpha: 18, lift: 0.8 },
    ...
    j10: jet({
        ...
        flight: { speed: 408, mach: 2.2, cd0: 0.0523, aoaLimiter: true, accel: 15, gLimit: 9, roll: 4.4, alpha: 28, lift: 1.04 },
    ...
    j8: jet({
        ...
        flight: { speed: 360, mach: 2.2, cd0: 0.0508, accel: 14, gLimit: 6.5, roll: 2.9, alpha: 20, lift: 0.85 },
    ...
    f5: jet({
        ...
        flight: { speed: 340, mach: 1.6, cd0: 0.0571, accel: 12.5, gLimit: 7.3, roll: 4.8, alpha: 24, lift: 1.0 },
    ...
    mirage: jet({
        ...
        flight: { speed: 388, mach: 2.2, cd0: 0.0485, accel: 13, gLimit: 7, roll: 4.0, alpha: 26, lift: 0.95 },
    ...
    jaguar: jet({
        ...
        flight: { speed: 374, mach: 1.6, cd0: 0.0553, accel: 11.5, gLimit: 7, roll: 3.4, alpha: 22, lift: 0.95 },
    ...
    su47: jet({
        ...
        flight: { speed: 384, mach: 1.65, cd0: 0.0451, aoaLimiter: true, accel: 15, gLimit: 9, roll: 4.2, alpha: 36, lift: 1.15 },
        health: 110, missiles: 8, gun: { damage: 12, rate: 13, ammo: 250, name: 'GSh-30-1 30mm' },
    ...
    b2: {
        ...
        flight: { speed: 289, mach: 0.95, aoaLimiter: true, afterburner: false, accel: 7, lift: 1.2, gLimit: 3, roll: 1.2, alpha: 16 },
    ...
    racer: {
        ...
        flight: { speed: 231, mach: 0.72, accel: 7.5, lift: 1.55, gLimit: 8, roll: 3.6, alpha: 18 },
    ...
    pitts: {
        ...
        flight: { speed: 95, mach: 0.35, accel: 6, lift: 3.4, gLimit: 6, roll: 7.0, alpha: 20 },
    ...
    cessna: {
        ...
        flight: { speed: 65, mach: 0.25, accel: 3.2, lift: 6.0, gLimit: 3.8, roll: 1.6, alpha: 16 },
    ...
    b737: {
        ...
        flight: { speed: 245, mach: 0.82, accel: 4.5, lift: 0.82, gLimit: 2.5, roll: 0.8, alpha: 15 },
    ...
    b747: {
        ...
        flight: { speed: 258, mach: 0.92, accel: 4, lift: 0.66, gLimit: 2.5, roll: 0.6, alpha: 14 },
